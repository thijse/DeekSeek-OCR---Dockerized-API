#!/usr/bin/env python3
"""
DeepSeek-OCR Gradio Interface (Simplified)
...
def process_pdf_interface(pdf_file, processing_mode, custom_prompt_text):
    """
    Process PDF file based on selected mode
    
    Args:
        pdf_file: Uploaded PDF file (can be path string or file object)
    ...
    # Process the PDF
    success, result = client.process_pdf(pdf_path, prompt)
...
# Create the Gradio interface using the Interface class (simpler, avoids bugs)
with gr.Blocks(title="DeepSeek-OCR PDF Converter", theme=gr.themes.Soft()) as demo:
    gr.Markdown("""
    ...
    **Supported Modes:**
    ...
    5. **Copy Results**: Select and copy the output text
    ...
    - Results can be copied directly from the text box
    """)
